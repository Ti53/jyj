...
if (!isset($_POST['submit'])) {
  ...
  if(!empty($_POST['email'])) {
  	...
  	if (!validEmail($email[2])) {
  	  ...
  	  $email[4] = '<strong><span style="color:#FF0000;">Email incorrecto</span></strong>';
	  }
  }
  else {
    $error = 1;
  ...
  if(empty($_POST['captcha_code'])) {
    ...
    if(!$valid) {
      $error = 1;
      $code[3] = 'color:#FF0000;';   
      $code[4] = '<strong><span style="color:#FF0000;">Codigo incorrecto</span></strong>';
    }
  }

  if ($error == 1) {
    $error_message = '<div style="font-weight:bold;font-size:90%;margin-bottom:5px;}">Por favor, corrija/introduzcalos campos en rojo.</div>';
...
<br /><br />(Por favor, introduzca el texto en la imagen de arriba.)<br />
<a href="#" onclick="document.getElementById('captcha').src = 'formfiles/contact-securimage_show.php?' + Math.random(); return false">Pulse aqui si no entiende el código.</a>
</td>
</tr>
<tr>
<td colspan="2" style="text-align:left; vertical-align:middle; padding:{$cell_padding}; font-size:90%; font-weight:bold;">Todos los campos son requeridos.</td>
</tr>
<tr>
<td colspan="2" style="text-align:left; vertical-align:middle; padding:{$cell_padding};"><input type="submit" name="submit" value="Enviar" style="border:1px solid #999;background:#E4E4E4;margin-top:5px;" id="submit_button" /></td>
</tr>
</table>
</form>
</div>
